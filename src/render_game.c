...
int render_game_frame(game_struct* mat) {
    ...
    for (int i = 0; i< ROW_QT; i++)
        for (int j = 0; j < COL_QT; j++) {
            draw_jewel(mat, &(mat->destroyed[i][j])); //draw destroyed/vanishing jewels
            draw_jewel(mat, &(mat->jewels[i][j])); //draw jewels
        }

    //redraw first selected to swap on top
    if (mat->swap2) draw_jewel(mat, mat->swap2);

    draw_ui(mat);

    if ((mat->state == NEW_LEVEL) || (mat->last_state == NEW_LEVEL)){ //draw new level screen;
    ...
    if (mat->state == END_GAME) { //draw end game screen
        al_draw_bitmap(mat->image.screen[TRANSPARENT_IMAGE], 0, 0, 0);
        al_draw_text(mat->font[GAME_FONT], al_map_rgb(255,255,255), (int)(SC_W/2)-90, SC_H-80, 0, "GAME OVER");
    }
    else if(mat->state == PAUSE) { //draw pause game screen
        al_draw_bitmap(mat->image.screen[TRANSPARENT_IMAGE], 0, 0, 0);
